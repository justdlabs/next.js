...
const CardHeader = ({ className, title, description, children, ...props }: HeaderProps) => (
	<div data-slot="card-header" className={twMerge("flex flex-col gap-y-1 px-6 py-5", className)} {...props}>
...
const CardDescription = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
	return <div {...props} data-slot="description" className={twMerge("text-muted-fg text-sm", className)} {...props} />
}
...
const CardFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
	return <div data-slot="card-footer" className={twMerge("flex items-center p-6 pt-0", className)} {...props} />
}
